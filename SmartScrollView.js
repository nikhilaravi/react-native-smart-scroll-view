...
import ReactNative, { View, ScrollView, Keyboard, Dimensions, LayoutAnimation, Platform } from 'react-native';
...
class SmartScrollView extends Component {
  ...
  _focusNode(ref) {
    const { scrollPosition, scrollWindowHeight } = this.state;
    const { scrollPadding, onRefFocus } = this.props;
  ...
  render() {
    ...
    return (
      <View ref={component => (this._container = component)} style={scrollContainerStyle} onLayout={e => (this._layout = e.nativeEvent.layout)}>
        <View>
          <ScrollView
